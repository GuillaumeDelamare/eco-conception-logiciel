\documentclass[12pt, screen, french]{report-rd-info}
...
\title{Projet de Recherche Et Developpement}
\subtitle{Analyse de la consommation de disques durs}
\authorA{Guillaume}{Delamare}
\authorB{Alter}{Ego}
\supervisor{Jean Pierre}{Guédon}
\institution{IRCCyN}
   ...
   % XXX pour un encadrement dans le cadre d'un autre organisme ; il faut alors fournir dans le répertoire "logos" les fichiers correspondant (XXX.pdf -- à défaut XXX.jpeg ou XXX.png -- pour pdflatex et XXX.eps pour latex)
   % laisser vide pour un encadrement qui relève d'un travail de recherche non affecté à une équipe.
\theme{\'Equipe d'accueil}
   % à fournir dans le cas d'un laboratoire
   % laisser vide autrement
\date{version 2.1 du 2 février 2010}
...
\begin{classification} % Modèle ACM : cf. http://www.acm.org/class/1998/
...
Pour chaque proposition étudiée\footnote{Substituez au marqueur << \emph{<Proposition $i$>} >> un titre explicite !}, sauf exception, le plan s'annonce de la même façon :
\begin{itemize}
...
\begin{figure}
	...
	\begin{picture}(1.2, 1.3)
	  ...
	  \put(.03, .98){$L$}
	\end{picture}
	\caption{Qualité des résultats obtenus}
	\label{fig:Courbes}
\end{figure}
...
Le résumé doit faire apparaître l'idée phare de l'article (ou les idées principales d'un ouvrage plus important -- il peut être étudié selon ses différents chapitres). Il faut rapporter les analyses, expérimentations et conclusions établies par les auteurs.
